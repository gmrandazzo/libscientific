...
void NewUPLSModel(UPLSMODEL** m)
{
  ...
  initArray(&(*m)->r2y_model);
  initArray(&(*m)->r2y_validation);
  initArray(&(*m)->q2y);
  initArray(&(*m)->sdep);
  initArray(&(*m)->sdec);
  initArray(&(*m)->recalculated_y);
  initArray(&(*m)->predicted_y);
  initArray(&(*m)->recalc_residuals);
  initArray(&(*m)->pred_residuals);
  
  initArray(&(*m)->q2y_yscrambling);
  initArray(&(*m)->sdep_yscrambling);
}

void DelUPLSModel(UPLSMODEL** m)
{
  DelMatrix(&(*m)->xscores);
  DelArray(&(*m)->xloadings);
  DelArray(&(*m)->xweights);
  ...
  DelArray(&(*m)->r2y_model);
  DelArray(&(*m)->r2y_validation);
  DelArray(&(*m)->q2y);
  DelArray(&(*m)->sdep);
  DelArray(&(*m)->sdec);
  DelArray(&(*m)->recalculated_y);
  DelArray(&(*m)->predicted_y);
  
  DelArray(&(*m)->recalc_residuals);
  DelArray(&(*m)->pred_residuals);
  
  DelArray(&(*m)->q2y_yscrambling);
  DelArray(&(*m)->sdep_yscrambling);
  
  xfree((*m));
}

int CheckPredArrays(array *X_, array *Y_)
{ 
  size_t i, j;
 ...
 * 
 * X_ = input Array for object descriptors
 * Y_ = input Array for Y response value
 * X = array MeanCentered and Autoscaled
 * Y = array MeanCentered and Autoscaled
  ...
  if(CheckArrays(X_, Y_) == 0){
    ...
    for(k = 0; k < X_->order; k++){
      ...
      for(j = 0; j < X_->m[k]->col; j++){
        if(X_->m[k]->row > 1){
          for(i = 0; i < X_->m[k]->row; i++){
            setArrayValue(X, k, i, j, getArrayValue(X_, k, i, j) - getMatrixValue(m->xcolaverage, j, k));
          }
        }
        else{
          for(i = 0; i < X_->m[k]->row; i++){
    ...
    */    
      ...
      for(k = 0; k < X->order; k++){
        for(j = 0; j < X->m[k]->col; j++){
          if(getMatrixValue(m->xcolscaling, j, k) == 0){
            ...
            for(i = 0; i < X->m[k]->row; i++){
              setArrayValue(X, k, i, j, getArrayValue(X, k, i, j)/getMatrixValue(m->xcolscaling, j, k));
            }
          }
        }
      }
    ...
    for(k = 0; k < X->order; k++)
      for(i = 0; i < X->m[k]->row; i++)
        for(j = 0; j < X->m[k]->col; j++)
          ssx += square(getArrayValue(X, k, i, j));

    #ifdef DEBUG
    puts("Processed X Array");
    PrintArray(X);
    #endif
    ...
    for(k = 0; k < Y_->order; k++){
      NewArrayMatrix(&Y, k, Y_->m[k]->row, Y_->m[k]->col);
      
        /* For Wold Geladi test enable this 
      for(i = 0; i < Y_->m[k]->row; i++)
        for(j = 0; j < Y_->m[k]->col; j++)
          setArrayValue(Y, k, i, j, getArrayValue(Y_, k, i, j));
     */   
    }
    ...
    for(k = 0; k < Y_->order; k++){
      ...
      for(j = 0; j < Y_->m[k]->col; j++){
        if(Y_->m[k]->row > 1){
          for(i = 0; i < Y_->m[k]->row; i++){
            setArrayValue(Y, k, i, j, getArrayValue(Y_, k, i, j) - getMatrixValue(m->ycolaverage, j, k));
          }
        }
        else{
          for(i = 0; i < Y_->m[k]->row; i++){
        ...
        for(j = 0; j < Y->m[k]->col; j++){
          if(getMatrixValue(m->ycolscaling, j, k) == 0){
            for(i = 0; i< Y->m[k]->row; i++){
              setArrayValue(Y, k, i, j, 0.f);
            }
          }
          else{
            for(i = 0; i < Y->m[k]->row; i++){
              setArrayValue(Y, k, i, j, getArrayValue(Y, k, i, j)/getMatrixValue(m->ycolscaling, j, k));
            }
          }
        }
      }
    ...
    for(pc = 0; pc < npc; pc++){
      ...
      for(i = 0; i < Y->m[order]->row; i++){
        setDVectorValue(u, i, getArrayValue(Y, order, i, column));
      }    
      ...
      while(1){
        ...
        */
        ...
        ArrayMatrixDotProduct(X, W, t_new);
...
/*         if(a/(t_new->size*DVectorDVectorDotProd(t_new, t_new)) < 1e-10){ */
        if(a/(t_new->size*DVectorDVectorDotProd(t_new, t_new)) < UPLSCONVERGENCE){
          ...
          ArrayAppendMatrix(&(m->xweights), W);
      ...
      /* step 8: Q = t'Y/t't */
      MatrixSet(Q, 0.f);
      DvectorArrayDotProduct(Y, t_new, Q);
      ...
      ArrayAppendMatrix(&(m->yloadings), Q);
      
      /* step 9: u = Y..Q/||Q||^2 */
      DVectorSet(u, 0.f);
      ArrayMatrixDotProduct(Y, Q, u);
      ...
      /* step 10: XLoadings => P = t'X/t't */
      DvectorArrayDotProduct(X, t_new, P);
      ...
      ArrayAppendMatrix(&(m->xloadings), P);
      ...
      for(k = 0; k < Y->order; k++){
        for(i = 0; i < Y->m[k]->row; i++){
          for(j = 0; j < Y->m[k]->col; j++){
            setArrayValue(Y, k, i, j, getArrayValue(Y, k, i, j) - (getDVectorValue(t_new, i)*getMatrixValue(Q, j, k)*getDVectorValue(m->b, pc)) );
          }
        }    
      }
    ...
    /*Recalculated y*/
    for(k = 0; k  < Y_->order; k++){
      AddArrayMatrix(&m->recalculated_y, 0, 0);
    }
    
    for(i = 1; i <= npc; i++){
      array *recalculated_y;
      initArray(&recalculated_y);
      ...
      for(k = 0; k  < recalculated_y->order; k++){
        for(j = 0; j < recalculated_y->m[k]->col; j++){
          ...
          ArrayAppendColumn(&m->recalculated_y, k, v);
          DelDVector(&v);
        }
      }
      DelArray(&recalculated_y);
    }
    
    /*Calculate recalculated residuals */
    for(k = 0; k < Y_->order; k++){
      AddArrayMatrix(&m->recalc_residuals, m->recalculated_y->m[k]->row, m->recalculated_y->m[k]->col);
      for(i = 0; i < m->recalculated_y->m[k]->row; i++){
        for(j = 0; j < m->recalculated_y->m[k]->col; j++){
          setArrayValue(m->recalc_residuals, k, i, j, getArrayValue(m->recalculated_y, k, i, j) - getArrayValue(Y_, k, i, (size_t)floor(j/npc)));
        }
      }
    }
...
{    
  if(CheckPredArrays(X_, m->xloadings) == 0){
    size_t pc, i, j , k;
    array *X;
    dvector *t;
    ...
    for(pc = 0; pc < npc; pc++){
      DVectorSet(t, 0.f);
      ArrayMatrixDotProduct(X, m->xweights->m[pc], t);
      ...
      for(k = 0; k < X->order; k++){
        for(i = 0; i < X->m[k]->row; i++){
          for(j = 0; j < X->m[k]->col; j++){
            setArrayValue(X, k, i, j, getArrayValue(X, k, i, j) - (getDVectorValue(t, i)*getMatrixValue(m->xloadings->m[pc], j, k)));
          }
        }    
      }  
    }
...
{    
  ...
  Y Loadings
  Array - order: 2
  Array No: 1 of row: 1; col: 2
    1.821           1.339
  Array No: 2 of row: 1; col: 2
    3.536           1.768
  ...
  */
  ...
  for(k = 0; k < m->yloadings->m[0]->col; k++){
    AddArrayMatrix(py, tscores->row, m->yloadings->m[0]->row );
  } 
    ...
    for(k = 0; k < (*py)->order; k++){
      for(i = 0; i <  tscores->row; i++){
        for(j = 0; j < (*py)->m[k]->col; j++){
          /*printf("[%u][%u][%u]  %f %f %f\n", (unsigned int)k, (unsigned int)i, (unsigned int)j, getDVectorValue(m->b, pc), getMatrixValue(tscores, i, pc), getArrayValue(m->yloadings, pc, j, k));*/
          setArrayValue((*py), k, i, j, getArrayValue((*py), k, i, j) +  (getDVectorValue(m->b, pc) * getMatrixValue(tscores, i, pc) * getArrayValue(m->yloadings, pc, j, k)));
        }
      }
    }    
  }
      ...
      for(j = 0; j < m->ycolaverage->row; j++){
        ...
        if(m->ycolscaling != NULL && m->ycolscaling->data != NULL
          ...
          for(i = 0; i < (*py)->m[k]->row; i++){
            setArrayValue((*py), k, i, j, getArrayValue((*py), k, i, j) * getMatrixValue(m->ycolscaling, j, k));
          }
        }
        for(i = 0; i < (*py)->m[k]->row; i++){
          setArrayValue((*py), k, i, j, getArrayValue((*py), k, i, j) + getMatrixValue(m->ycolaverage, j, k));
        }
      }
    }
  }
...
void UPLSRSquared(array *X_, array *Y_, UPLSMODEL *m, size_t npc, dvector** r2x, array** r2y, array **sdec)
{
  ...
  for(pc = 1; pc <= npc; pc++){
    ...
    for(k = 0; k < recalcy->order; k++){
      ...
      for(j = 0; j < recalcy->m[k]->col; j++){
        for(i = 0; i < recalcy->m[k]->row; i++){
          n += square(getArrayValue(recalcy, k, i, j) - getArrayValue(Y_, k, i, j));
          d += square(getArrayValue(Y_, k, i, j) - getMatrixValue(m->ycolaverage, j, k));
        }
    ...
    ArrayAppendMatrix(sdec, sdec_);
    ArrayAppendMatrix(r2y, r2y_);
      ...
      for(k = 0; k < recalcx->order; k++){
        for(j = 0; j < recalcx->m[k]->col; j++){
          for(i = 0; i < recalcx->m[k]->row; i++){
            n += square(getArrayValue(X_, k, i, j) - getArrayValue(recalcx, k, i, j));
            d += square(getArrayValue(X_, k, i, j) - getMatrixValue(m->xcolaverage, j, k));
          }
        }
      }
...
void UPLSRSquared_SSErr_SStot(array *X_, array *Y_, UPLSMODEL *m, size_t npc, dvector** xss_err, dvector **xss_tot, array** yss_err, array** yss_tot, array** predicted_y)
{
  ...
  if(predicted_y != NULL){
    for(i = 0; i < Y_->order; i++){
      AddArrayMatrix(predicted_y, Y_->m[i]->row, 0);
    }
  }
  ...
  for(pc = 1; pc <= npc; pc++){
    ...
    ArrayAppendMatrix(yss_err, yss_err_);
    ArrayAppendMatrix(yss_tot, yss_tot_);
                        ...
                        array **q2y, array **sdep, ssignal *s)
{
  ...
  double temp;
  matrix *sorty, *gid;
  array *randomY, *sorted_y_id;
  array *tmpq2, *tmpsdep;
  
  srand(X_->order*X_->m[0]->row*X_->m[0]->col*block);  
  NewArray(&randomY, Y_->order);
  NewArray(&sorted_y_id, Y_->order);
  for(i = 0; i < Y_->order; i++){
    NewArrayMatrix(&randomY, i, Y_->m[i]->row, Y_->m[i]->col);
    NewArrayMatrix(&sorted_y_id, i, Y_->m[i]->row, Y_->m[i]->col);
  }
  ...
  DelMatrix(&gid);
  DelArray(&randomY);
  DelArray(&sorted_y_id);  
}

void UPLSRandomGroupsCV(array *X_, array *Y_, size_t xautoscaling, size_t yautoscaling, size_t npc, size_t group, size_t iterations, dvector **r2x, array **q2y, array **sdep, array **predicted_y, array **pred_residuals, ssignal *s)
{
  if(npc > 0 && X_->m[0]->row == Y_->m[0]->row && group > 0 && iterations > 0){
    ...
    array *predictyss_err;
    array *predictyss_tot;
    array *predicty;
    ...
    initArray(&sumyss_err);
    initArray(&sumyss_tot);
    ...
    if(predicted_y != NULL){
      if(pred_residuals != NULL){
        for(k = 0; k < Y_->order; k++){
          AddArrayMatrix(predicted_y, Y_->m[k]->row, Y_->m[k]->col*npc); /* each component have my->col ypsilon */
          AddArrayMatrix(pred_residuals, Y_->m[k]->row, Y_->m[k]->col*npc); /* each component have my->col ypsilon */
        }
      }
      else{
        for(k = 0; k < Y_->order; k++){
      ...
      if(s != NULL && (*s) == SIGSCIENTIFICSTOP){
        ...
        */
        ...
        for(g = 0; g < gid->row; g++){ /*For aeach group */  
          ...
          NewArray(&subX, X_->order);
          NewArray(&subY, Y_->order);
          
          for(k = 0; k < X_->order; k++){
            NewArrayMatrix(&subX, k, n, X_->m[k]->col);
          }
          
          for(k = 0; k < Y_->order; k++){
            NewArrayMatrix(&subY, k, n, Y_->m[k]->col);
          }
          ...
          /*Allocate the */
          NewArray(&predictX, X_->order);
          NewArray(&realY, Y_->order);
          
          for(k = 0; k < X_->order; k++){
            NewArrayMatrix(&predictX, k, n, X_->m[k]->col);
          }
          
          for(k = 0; k < Y_->order; k++){
            NewArrayMatrix(&realY, k, n, Y_->m[k]->col);
          }
                ...
                if(a != -1){
                  for(k = 0; k < subX->order; k++){
                    for(n = 0; n < X_->m[k]->col; n++){
                      setArrayValue(subX, k, l, n, getArrayValue(X_, k, a, n));
                    }
                  }
                  
                  for(k = 0; k < subY->order; k++){
                    for(n = 0; n < Y_->m[k]->col; n++){
                      setArrayValue(subY, k, l, n, getArrayValue(Y_, k, a, n));
                    }
                  }
                  l++;
                }
          ...
          for(j = 0, l = 0; j < gid->col; j++){
            ...
            if(a != -1){
              for(k = 0; k < predictX->order; k++){
                for(n = 0; n < X_->m[k]->col; n++){
                  setArrayValue(predictX, k, l, n, getArrayValue(X_, k, a, n));
                }
              }
              /* copy the real value Y_ to realY */
              for(k = 0; k < realY->order; k++){
                for(n = 0; n < Y_->m[k]->col; n++){
                  setArrayValue(realY, k, l, n, getArrayValue(Y_, k, a, n));
                }
              }
              l++;
            }
          ...
          */
          ...
          initArray(&predictyss_err);
          initArray(&predictyss_tot);
          ...
          if(iterations_ == 0 && g == 0){
            ...
            ArrayCopy(predictyss_err, &sumyss_err);
            ArrayCopy(predictyss_tot, &sumyss_tot);
            
            ArrayCopy(predictyss_err, sdep);
            ...
            for(k = 0; k < predictyss_err->order; k++){
              for(j = 0; j < predictyss_err->m[k]->col; j++){
                for(i = 0; i < predictyss_err->m[k]->row; i++){
                  setArrayValue((*sdep), k, i, j, getArrayValue((*sdep), k, i, j)/l);
                }
              }
            }
                ...
                if(a != -1){
                  ...
                  for(k = 0; k < predicty->order; k++){
                    for(n = 0; n < predicty->m[k]->col; n++){
                      setArrayValue((*predicted_y), k, a, n, (getArrayValue((*predicted_y), k, a, n) + getArrayValue(predicty, k, l, n)));
                    }
                  }
                  l++;
                }
            ...
            for(k = 0; k < predictyss_err->order; k++){
              for(j = 0; j < predictyss_err->m[k]->col; j++){
                for(i = 0; i < predictyss_err->m[k]->row; i++){
                  setArrayValue(sumyss_err, k, i, j, getArrayValue(sumyss_err, k, i, j) + getArrayValue(predictyss_err, k, i, j));
                  setArrayValue(sumyss_tot, k, i, j, getArrayValue(sumyss_tot, k, i, j) + getArrayValue(predictyss_tot, k, i, j));
                }
              }
            }
            ...
            for(k = 0; k < predictyss_err->order; k++){
              for(j = 0; j < predictyss_err->m[k]->col; j++){
                for(i = 0; i < predictyss_err->m[k]->row; i++){
                  setArrayValue((*sdep), k, i, j, getArrayValue((*sdep), k, i, j) + (getArrayValue(predictyss_err, k, i, j)/l));
                }
              }
            }
          ...
          DelArray(&predictyss_err);
          DelArray(&predictyss_tot);
    ...
    for(k = 0; k < sumyss_err->order; k++){
      ArrayAppendMatrix(q2y, sumyss_err->m[k]);
      for(j = 0; j < sumyss_err->m[k]->col; j++){
        for(i = 0; i < sumyss_err->m[k]->row; i++){
          setArrayValue((*q2y), k, i, j, 1 - (getArrayValue((*q2y), k, i, j)/getArrayValue(sumyss_tot, k, i, j)));
        }
      }
    }
    ...
    for(k = 0; k < (*sdep)->order; k++){
      for(j = 0; j < (*sdep)->m[k]->col; j++){
        for(i = 0; i < (*sdep)->m[k]->row; i++){
          /*setArrayValue((*sdep), k, i, j, sqrt(getArrayValue((*sdep), k, i, j)/(group*iterations)));*/
          setArrayValue((*sdep), k, i, j, sqrt(getArrayValue((*sdep), k, i, j)/(iterations)));
        }
      }
    }
        ...
        for(k = 0; k < (*predicted_y)->order; k++){
          for(j = 0; j < (*predicted_y)->m[k]->col; j++){
            for(i = 0; i < (*predicted_y)->m[k]->row; i++){
              setArrayValue((*predicted_y), k, i, j, getArrayValue((*predicted_y), k, i, j)/getUIVectorValue(predictcount, i));
              setArrayValue((*pred_residuals), k, i, j, getArrayValue((*predicted_y), k, i, j) - getArrayValue(Y_, k, i, (size_t)floor(j/npc)));
            }
          }
        }
      }
 ...
 */
void UPLSLOOCV(array* X_, array* Y_, size_t xautoscaling, size_t yautoscaling, size_t npc, dvector** r2x, array** q2y, array** sdep, array** predicted_y, array **pred_residuals, ssignal *s)
{
  if(npc > 0 && X_->m[0]->row == Y_->m[0]->row){
    ...
    for(k = 0; k < X_->order; k++){
      NewArrayMatrix(&subX, k, X_->m[k]->row-1, X_->m[k]->col);
      NewArrayMatrix(&predictX, k, 1, X_->m[k]->col);
    }
    
    for(k = 0; k < Y_->order; k++){
      NewArrayMatrix(&subY, k, Y_->m[k]->row-1, Y_->m[k]->col);
      NewArrayMatrix(&realY, k, 1, Y_->m[k]->col);
    }
          ...
          for(j = 0; j < X_->m[k]->row; j++){
            if(j != model){
              for(l = 0; l < X_->m[k]->col; l++){
                setArrayValue(subX, k, m, l, getArrayValue(X_, k, j, l));
              }
              m++;
            }
            else{
              for(l = 0; l < X_->m[k]->col; l++){
                setArrayValue(predictX, k, 0, l, getArrayValue(X_, k, j, l));
              }
            }
          }
        }
          ...
          for(j = 0; j < Y_->m[k]->row; j++){
            if(j != model){
              for(l = 0; l < Y_->m[k]->col; l++){
                setArrayValue(subY, k, m, l, getArrayValue(Y_, k, j, l));
              }
              m++;
            }
            else{
              for(l = 0; l < Y_->m[k]->col; l++){
                setArrayValue(realY, k, 0, l, getArrayValue(Y_, k, j, l));
              }
            }
          }
        }
            ...
            if(pred_residuals != NULL){
              for(k = 0; k < predicty->order; k++){
                ...
                for(j = 0; j < predicty->m[k]->col; j++){
                  setArrayValue((*predicted_y), k, model, j, getArrayValue(predicty, k, 0, j));
                  setArrayValue((*pred_residuals), k, model, j, getArrayValue(predicty, k, 0, j) - getArrayValue(Y_, k, model, (size_t)floor(j/npc)));
                }
                /*}*/
              }
            }
    ...
    for(k = 0; k < (*sdep)->order; k++){
      for(j = 0; j < (*sdep)->m[k]->col; j++){
        for(i = 0; i < (*sdep)->m[k]->row; i++){
          setArrayValue((*sdep), k, i, j, sqrt(getArrayValue((*sdep), k, i, j)/X_->m[0]->row));
        }
      }
    }
  ...
  for(k = 0; k < rq2->order; k++){
    for(i = 0; i < rq2->m[k]->row; i++){
      for(j = 0; j < rq2->m[k]->col; j++){
        setMatrixValue(rq2mean, k, i, getMatrixValue(rq2mean, k, i) + getArrayValue(rq2, k, i, j));
      }
    }
  }
...
void UPLSCostPopulation(array *ax, array *ay, 
                      array *px, array *py, 
                      uivector *popvector, 
                      ...
                      double *r2, double *q2, ssignal *s)
{
  ...
  double r2_, q2_;
  dvector *r2x;
  array *subax, *subpx, *r2y, *sdec, *q2y, *sdep;
  UPLSMODEL *m;
  ...
  NewArray(&subax, ax->order);
  for(k = 0; k < ax->order; k++){
    NewArrayMatrix(&subax, k, ax->m[k]->row, subsize);
  }
    ...
    for(i = 0; i < ax->m[k]->row; i++){
      ...
      for(j = 0; j < ax->m[k]->col; j++){
        if(getUIVectorValue(popvector, j) == 1){
          setArrayValue(subax, k, i, cc, getArrayValue(ax, k, i, j));
          cc++;
        }
        else{
          continue; 
  ...
  if(px != NULL && py != NULL && validation_type == 0){ /* cost = R^2*/
    ...
    NewArray(&subpx, px->order);
    for(k = 0; k < px->order; k++){
      NewArrayMatrix(&subpx, k, px->m[k]->row, subsize);
    }
    ...
    for(k = 0; k < px->order; k++){
      for(i = 0; i < px->m[k]->row; i++){
        ...
        for(j = 0; j < px->m[k]->col; j++){
          if(getUIVectorValue(popvector, j) == 1){
            setArrayValue(subpx, k, i, cc, getArrayValue(px, k, i, j));
            cc++;
          }
          else{
            continue; 
    ...
    if(r2y->order > 0){
      ...
      for(i = 0; i < r2y->m[cutoff]->row; i++){
        for(j = 0; j < r2y->m[cutoff]->col; j++){
          r2_ += getArrayValue(r2y, cutoff, i, j);
          cc++;
        }
      }
      r2_ /=cc;
    ...
    if(q2y->order > 0){
      ...
      for(i = 0; i < q2y->m[cutoff]->row; i++){
        for(j = 0; j < q2y->m[cutoff]->col; j++){
          q2_ += getArrayValue(q2y, cutoff, i, j);
          cc++;
        }
      }
      q2_ /=cc;
      ...
      for(i = 0; i < m->r2y_model->m[cutoff]->row; i++){
        for(j = 0; j < m->r2y_model->m[cutoff]->col; j++){
          r2_ += getArrayValue(m->r2y_model, cutoff, i, j);
          cc++;
        }
      }
      r2_ /=cc;
...
{   
  ...
  puts("---------------");
  puts("X Loadings");
  PrintArray(m->xloadings);
  
  puts("Y Loadings");
  PrintArray(m->yloadings);
  
  
  puts("X Weights");
  PrintArray(m->xweights);
  ...
  puts("Recalculated Y");
  PrintArray(m->recalculated_y);
  
  puts("Recalculated Residuals");
  PrintArray(m->recalc_residuals);
}
